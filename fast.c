...
//引数の座標x,yに向かって最短走行する
	...
	while((mypos.x != x) || (mypos.y != y)){			//ゴールするまで繰り返す
		...
		switch(get_nextdir(x,y,MASK_SECOND,&glob_nextdir))	//次に行く方向を戻り値とする関数を呼ぶ
		{
			...
			case right:
				straight_NC(SECTION*straight_count,FAST_ACCEL,FAST_SPEED,0.0);
				turn(90,TURN_ACCEL,TURN_SPEED,RIGHT);				//右に曲がって
			...
			case left:
				...
				if(isWallonSide(right)){
					back(-40);
					straight_NC(HALF_SECTION+20,FAST_ACCEL,FAST_SPEED,0.0);
				}else{
					straight_NC(HALF_SECTION,FAST_ACCEL,FAST_SPEED,0.0);
				}
				straight_count = 1;			//走る直線の距離をリセット
				break;
